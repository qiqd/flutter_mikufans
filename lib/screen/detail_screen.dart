...
class _DetailScreenState extends State<DetailScreen>
  ...
  void _saveHistory() {
    ...
    if (history == null) {
      history = History(
        ...
        lastViewAt: DateTime.now().add(const Duration(days: -1)),
      );
    } else {
      history!.isLove = isLove;
      history!.episodeIndex = episodeIndex;
      history!.lastViewAt = DateTime.now().add(const Duration(days: -1));
    }
  ...
  Widget build(BuildContext context) {
    return VisibilityDetector(
      ...
      child: Column(
        children: [
          TitleBar(backgroundColor: Theme.of(context).scaffoldBackgroundColor),
          Expanded(
            child: Scaffold(
              appBar: AppBar(
                                ...
                                onPressed: () {
                                  setState(() {
                                    for (var item in _detail!.sources) {
                                      item.episodes = item.episodes.reversed
                                          .toList();
                                    }
                                  });
                                },
